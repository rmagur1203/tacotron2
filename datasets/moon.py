...
def _process_utterance(out_dir, wav_path, text, hparams):
    ...
    if hparams.input_type == "mulaw-quantize":
        ...
        constant_values = audio.mulaw(0.0, hparams.quantize_channels)
    ...
    # hparams.max_mel_frames = 1000, hparams.clip_mels_length = True
    ...
    if npz_flag:
        ...
        np.save(
            os.path.join(out_dir, mel_filename), mel_spectrogram.T, allow_pickle=False
        )
        np.save(
            os.path.join(out_dir, linear_filename),
