...
def get_path_dict(data_dirs, hparams, config, data_type, n_test=None, rng=np.random.RandomState(123)):
    ...
    for data_dir in data_dirs:  # ['datasets/moon\\data']
        ...
        if not config.skip_path_filter:
            ...
            new_items = [(path, n) for path, n, n_tokens in items if min_n_frame <=
                         n <= max_n_frame and n_tokens >= hparams.min_tokens]
        ...
        # ['datasets/moon\\data\\001.0621.npz', 'datasets/moon\\data\\003.0229.npz', ...]
...
class DataFeederTacotron2(threading.Thread):
    ...
    # batches_per_group = 32 or 8,  data_type: 'train' or 'test'
    def __init__(self, coordinator, data_dirs, hparams, config, batches_per_group, data_type, batch_size):
        ...
        # Load metadata:
        self.path_dict = get_path_dict(data_dirs, self._hp, config, self.data_type,
                                       n_test=self.batch_size, rng=self.rng)  # data_dirs: ['datasets/moon\\data']

        self.data_dirs = list(self.path_dict.keys())  # ['datasets/moon\\data']
        self.data_dir_to_id = {data_dir: idx for idx, data_dir in enumerate(
            self.data_dirs)}  # {'datasets/moon\\data': 0}

        # {'datasets/moon\\data': 1.0}
        data_weight = {data_dir: 1. for data_dir in self.data_dirs}

        # 'main_data': ['']
        if self._hp.main_data_greedy_factor > 0 and any(main_data in data_dir for data_dir in self.data_dirs for main_data in self._hp.main_data):
        ...
        # 각 data들의 weight sum이 1이 되도록...
        self.data_ratio = {data_dir: weight /
                           weight_Z for data_dir, weight in data_weight.items()}
        ...
        if self.is_multi_speaker:
            self._placeholders.append(tf.placeholder(
    ...
    def _enqueue_next_group(self):
        ...
        # 4 or 5  min_n_frame,max_n_frame 계산에 사용되었던...
        r = self._hp.reduction_factor

        # 'test'에서는 static_batches를 사용한다. static_batches는 init에서 이미 만들어 놓았다.
            ...
            for data_dir in self.data_dirs:
                if self._hp.initial_data_greedy:
                    if self._step < self._hp.initial_phase_step and any("krbook" in data_dir for data_dir in self.data_dirs):
                        data_dir = [
                            data_dir for data_dir in self.data_dirs if "krbook" in data_dir][0]
                ...
                if self._step < self._hp.initial_phase_step:  # 'initial_phase_step': 8000
                    ...
                    example = [self._get_next_example(data_dir) for _ in range(
                        int(n * self._batches_per_group // len(self.data_dirs)))]
                else:
                    example = [self._get_next_example(data_dir) for _ in range(
                        int(n * self._batches_per_group * self.data_ratio[data_dir]))]
        ...
        for batch in batches:  # batches는 batch의 묶음이다.
            ...
            # _prepare_batch에서 batch data의 길이를 맞춘다. return 순서 = placeholder순서
            feed_dict = dict(zip(self._placeholders, _prepare_batch(
                batch, r, self.rng, self.data_type)))
    ...
    def _get_next_example(self, data_dir):
        ...
        while True:
            ...
            try:
                if os.path.exists(data_path):
                    # data속에는 "linear","mel","tokens","loss_coeff"
        ...
        # mel_target은 [xx,80]으로 data마다 len이 다르다.  len에 따라 [0,...,0]
...
def _prepare_batch(batch, reduction_factor, rng, data_type=None):
    ...
    # batch에 있는 data들 중, 가장 긴 data의 길이에 맞게 padding한다.
    inputs = _prepare_inputs([x[0] for x in batch])
    # batch_size, [37, 37, 32, 32, 38,..., 39, 36, 30]
    input_lengths = np.asarray([len(x[0]) for x in batch], dtype=np.int32)
    # batch_size, [1,1,1,,..., 1,1,1]
    loss_coeff = np.asarray([x[1] for x in batch], dtype=np.float32)

    # ---> (32, 175, 80) max length는 reduction_factor의  배수가 되도록
    mel_targets = _prepare_targets([x[2] for x in batch], reduction_factor)
    # ---> (32, 175, 1025)  max length는 reduction_factor의  배수가 되도록
    linear_targets = _prepare_targets([x[3] for x in batch], reduction_factor)
    stop_token_targets = _prepare_stop_token_targets(
        [x[4] for x in batch], reduction_factor)

    if len(batch[0]) == 7:  # is_multi_speaker = True인 경우
        speaker_id = np.asarray([x[5] for x in batch],
                                dtype=np.int32)   # speaker_id로 list 만들기
        return (inputs, input_lengths, loss_coeff, mel_targets, linear_targets, stop_token_targets, speaker_id)
    else:
        # ('inputs' 'input_lengths' 'loss_coeff' 'mel_targets' 'linear_targets')
        return (inputs, input_lengths, loss_coeff, mel_targets, linear_targets, stop_token_targets)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip_path_filter', type=str2bool,
                        default=True, help='Use only for debugging')
    config = parser.parse_args()

    coord = tf.train.Coordinator()
    data_dirs = ['D:\\hccho\\Tacotron-Wavenet-Vocoder-hccho\\data\\moon']
    mydatafeed = DataFeederTacotron2(
    ...
    with tf.Session() as sess:
        try:
            ...
            while not coord.should_stop():
                a, b, c, d = sess.run([mydatafeed.inputs, mydatafeed.input_lengths,
                                      mydatafeed.mel_targets, mydatafeed.stop_token_targets])
                ...
                print('-'*10)
                a, b, c = sess.run(
                    [mydatafeed.inputs, mydatafeed.input_lengths, mydatafeed.mel_targets])
