...
def get_path_dict(
    data_dirs, hparams, config, data_type, n_test=None, rng=np.random.RandomState(123)
):
    # Load metadata:
    path_dict = {}
...
class DataFeederTacotron2(threading.Thread):
    ...
    def __init__(
        ...
        data_weight = {data_dir: 1.0 for data_dir in self.data_dirs}

        # 'main_data': ['']
        if self._hp.main_data_greedy_factor > 0 and any(
            main_data in data_dir
    ...
    def _enqueue_next_group(self):
        ...
        if self.static_batches is not None:
            ...
            for data_dir in self.data_dirs:
                ...
                if (
                    self._step < self._hp.initial_phase_step
        ...
        log(
            "Generated %d batches of size %d in %.03f sec"
    ...
    def _get_next_example(self, data_dir):
        ...
        while True:
            ...
            if (
                self.min_n_frame <= data["linear"].shape[0] <= self.max_n_frame
                and len(data["tokens"]) > self.min_tokens
            ):
                break
        ...
        stop_token_target = np.asarray([0.0] * len(mel_target))
...
def _prepare_batch(batch, reduction_factor, rng, data_type=None):
    ...
    if len(batch[0]) == 7:  # is_multi_speaker = True인 경우
        speaker_id = np.asarray(
            [x[5] for x in batch], dtype=np.int32
        )  # speaker_id로 list 만들기
        return (
            inputs,
            input_lengths,
            loss_coeff,
...
def _prepare_stop_token_targets(targets, alignment):
    max_len = max((len(t) for t in targets)) + 1
    return np.stack(
        [_pad_stop_token_target(t, _round_up(max_len, alignment)) for t in targets]
    )
...
def _pad_target(t, length):
    ...
    # (169, 80) ==> (length, 80)
    return np.pad(
        t, [(0, length - t.shape[0]), (0, 0)], mode="constant", constant_values=_pad
    )
...
def _pad_stop_token_target(t, length):
    return np.pad(
        t, (0, length - t.shape[0]), mode="constant", constant_values=_stop_token_pad
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_path_filter", type=str2bool, default=True, help="Use only for debugging"
    )
        ...
        try:
            ...
            while not coord.should_stop():
                a, b, c, d = sess.run(
                    [
                        mydatafeed.inputs,
                        mydatafeed.input_lengths,
                        mydatafeed.mel_targets,
                        mydatafeed.stop_token_targets,
                    ]
                )
