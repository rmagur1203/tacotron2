...
class Synthesizer():
    def __init__(self,
        ...
        with tf.variable_scope('model') as scope:
            ...
            self.model.initialize(inputs=inputs, input_lengths=input_lengths,
                                  num_speakers=self.num_speakers, speaker_id=speaker_id, is_training=False)
            self.wav_output = inv_spectrogram_tensorflow(
                self.model.linear_outputs, hparams)

    def load(self, checkpoint_path, checkpoint_step=None):
    ...
    def synthesize(self,
        ...
        def plot_and_save_parallel(wavs, alignments, mels):
            ...
            items = list(
                enumerate(zip(wavs, alignments, paths, texts, sequences, mels)))
        ...
        feed_dict = {self.model.inputs: sequences,
...
def plot_graph_and_save_audio(args,
    ...
    if librosa_trim and end_of_sentence:
        yt, index = librosa.effects.trim(
            audio_out, frame_length=5120, hop_length=256, top_db=50)
...
def get_most_recent_checkpoint(checkpoint_dir, checkpoint_step=None):
    if checkpoint_step is None:
        checkpoint_paths = [path for path in glob(
            "{}/*.ckpt-*.data-*".format(checkpoint_dir))]
        idxes = [int(os.path.basename(path).split('-')[1].split('.')[0])
                 for path in checkpoint_paths]

        max_idx = max(idxes)
    else:
        max_idx = checkpoint_step
    lastest_checkpoint = os.path.join(
        checkpoint_dir, "model.ckpt-{}".format(max_idx))
    ...
    if not end_of_sentence and post_word_num > 0:
        ...
        if True:  # attention based split
            if end_of_sentence:
                ...
                surplus_decomposed_text = decompose_ko_text(
                    "".join(text.split()[-1]))
                end_idx = len(decomposed_text.replace(
                    surplus_decomposed_text, '')) - 1
...
if __name__ == "__main__":
    ...
    synthesizer.load(config.load_path, config.num_speakers,
                     config.checkpoint_step, inference_prenet_dropout=False)
